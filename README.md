...
- 支持临时静音开关：群内 `.mute`（无参数不限时）/ `.unmute`，或本地 Shell 控制台开启；静音期间提醒暂存，群内解除或限时到期后汇总发出
- 支持本地终端 Shell 控制台：与 bot 同进程读 stdin，命令输出仅本地、不进群
...
- 解除并汇总：群内 `.unmute`（会汇总发出延后提醒）
- 终端 `unmute`：仅在本地解除静音状态，**不向群发送任何命令回执**；延后提醒需群内 `.unmute` 或等待夜间汇总
...
| `mute <分钟>` | 限时静音 |
| `unmute` | 本地解除静音（不向群发送；延后提醒需群内 `.unmute`） |
