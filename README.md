...
- 支持临时静音开关：群内 `.mute` / `.unmute` 或本地 Shell 控制台开启；静音期间提醒暂存，解除或到期后汇总发出
- 支持本地终端 Shell 控制台：与 bot 同进程读 stdin，可查状态、列客户、静音/解除、重载配置
...
| `.mute [分钟]` | 临时静音，暂停提醒并暂存（默认 60 分钟） | `.mute 30` | 否 |
| `.unmute` | 解除静音，并汇总发出静音期间的延后提醒 | `.unmute` | 否 |
...
- 开启：内部群 `.mute`（默认 60 分钟）或 `.mute 30`；也可在本地 Shell 控制台执行 `mute 30`
- 解除：`.unmute` / `unmute`；也可等待静音到期由巡检任务自动解除
...
- 持久化：`mute_until` 写入 `state_file`，程序重启后静音状态仍有效

## 本地 Shell 控制台

bot 启动后会在同一进程读取终端 stdin。交互式运行（或 `screen`/`tmux`）时可直接敲命令；在 systemd 等无 stdin 环境下控制台会自动禁用，不影响 bot 主流程。
