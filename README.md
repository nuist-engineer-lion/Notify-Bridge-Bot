...
| `.more <QQ\|all>` | 不引用时，按客户 QQ 或队列全部拉取历史合并转发 | `.more all` | 否（用 QQ/all） |
...
- **目标解析优先级：** 带 `Reply` 时**始终**走引用路径（`.say` 后文本不会被当成 QQ/all）。无引用且命令为 `.say/.bye/.close/.more` 时，第一个参数为客户 QQ 或 `all`。
- **`all` = 当前待回复队列**（不是全部好友）；群内与终端均支持；`.say all` 必须附带内容，不支持等待输入。
...
| `more` | **终端不支持**；请在通知群使用 `.more`（引用消息、`.more <QQ>` 或 `.more all`） |
| `quit` / `exit` / `stop` | 优雅停止 bot（保存状态并退出） |

终端说明：

- **命令 ACK 只写本地**，不向 QQ 群发送命令回执；`unmute` 的汇总提醒、以及群内 `.more <QQ>` 的合并转发属于业务消息，不在「命令 ACK」范围内。
- **终端没有 `more`**；历史合并转发只通过群内 `.more` / `.more <QQ>` / `.more all` 完成。
- **`all` 仅指当前待回复队列**，不是全部好友；群内与终端均支持 `all`（终端仍无 `more`）。
- `say` / `bye` 在客户端未运行时会本地拒绝；`close` 不要求客户端在线。

停止 bot 也可使用终端 `Ctrl+C`（或向进程发送 `SIGTERM`）。触发后会执行**优雅关停**：
