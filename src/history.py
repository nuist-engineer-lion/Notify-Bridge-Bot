...
def _failed_events_file() -> str:
    return os.path.join(cfg.ARCHIVE_DIR, "failed_events.jsonl")
...
def _dump_failed(event_type: str, record: dict) -> None:
    ...
    try:
        failed_path = _failed_events_file()
...
async def ensure_session(uid: int) -> int | None:
    ...
    data = cfg.unreplied_customers.get(uid)
    if data is None:
        return None
...
async def record_bot_send(uid: int, message_id: int | None, kind: str, message, ok: bool = True,
    ...
    if session_id is not None:
        ...
        if sid is None:
            return
    await _safe_record(sid, "bot_send", actor_uid=int(cfg.client.self_id), message_id=message_id,
...
async def recover_sessions() -> None:
    ...
    recovered = created = 0
    for uid, data in cfg.unreplied_customers.items():
    ...
    queue_sids = {d["session_id"] for d in cfg.unreplied_customers.values() if d.get("session_id")}
    try:
